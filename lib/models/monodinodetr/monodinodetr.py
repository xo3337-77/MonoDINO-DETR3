...
from .depth_predictor import DepthAnythingPredictor

def _get_clones(module, N):
    return nn.ModuleList([copy.deepcopy(module) for i in range(N)])


class MonoDINODETR(nn.Module):
    """ This is the MonoDINO-DETR module that performs monocualr 3D object detection """
    def __init__(self, backbone, depthaware_transformer, depth_predictor, num_classes, num_queries, num_feature_levels,
                 ...
                 depth_predict_with_anything=False):
        ...
        Parameters:
            ...
            two_stage: two-stage MonoDINO-DETR
        """
        super().__init__()
        ...
        # For depthanything
        self.depth_predict_with_anything = depth_predict_with_anything
        self.depth_predictor_2 = depth_predictor
        ...
        if num_feature_levels > 1:
            ...
            in_channels = backbone.embed_dim     # 384
            out_channels = backbone.num_channels    # [48, 96, 192, 384]
            
            # first level
            # input_proj_list.append(nn.Sequential(
            #     nn.Conv2d(in_channels, hidden_dim, kernel_size=1),
            #     # nn.ConvTranspose2d(out_channels[0], hidden_dim, kernel_size=8, stride=8, padding=0),
            ...
            input_proj_list.append(nn.Sequential(
                ...
                nn.ConvTranspose2d(out_channels[1], hidden_dim, kernel_size=4, stride=4, padding=0),
                nn.GroupNorm(32, hidden_dim)
            ))
            ...
            input_proj_list.append(nn.Sequential(
                ...
                nn.ConvTranspose2d(out_channels[2], hidden_dim, kernel_size=2, stride=2, padding=0),
                nn.GroupNorm(32, hidden_dim)
            ))
        ...
        for proj in self.input_proj:
            ...
            if isinstance(proj[1], nn.ConvTranspose2d):
    ...
    def forward(self, images, calibs, targets, img_sizes, dn_args=None):
        ...
        if len(outputs) == 2:
            ...
            features, pos, depth_output = outputs
        else:
            raise ValueError("Backbone returned an unexpected number of outputs.")
        ...
        if self.depth_predict_with_anything:
            depth_features = depth_output
            pred_depth_map_logits, depth_pos_embed, weighted_depth, depth_pos_embed_ip = self.depth_predictor_2(depth_features, masks[-1], pos[-1])
        else:
            pred_depth_map_logits, depth_pos_embed, weighted_depth, depth_pos_embed_ip = self.depth_predictor_2(srcs, masks[1], pos[1])
...
class SetCriterion(nn.Module):
    """ This class computes the loss for MonoDINO-DETR.
    ...
    def loss_depth_map(self, outputs, targets, indices, num_boxes):
        ...
        gt_boxes2d = torch.cat([t['boxes'] for t in targets], dim=0) * torch.tensor([37, 37, 37, 37], device='cuda')
...
def build(cfg):
    ...
    depth_predictor = DepthAnythingPredictor(cfg)
    ...
    model = MonoDINODETR(
        ...
        depth_predict_with_anything=cfg['depth_predict_with_anything'],
        )
